...
class DistributedWorker:
    ...
    def run(self):
        ...
        try:
            while self.running:
                # Worker waits for coordinator to send inference requests
                # The actual work happens in the DistributedMLXInference class
                # when the coordinator calls broadcast() to notify workers
                
                # Check for messages from coordinator
                try:
                    # This is a simplified worker loop
                    # In practice, the worker responds to specific gRPC calls
