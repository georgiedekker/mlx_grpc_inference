...
async def gpu_info():
    ...
    def get_local_gpu_info(device_id: str, rank: int):
        ...
        return {
            ...
            "test_computation_time": compute_time,
    ...
    if device and device.role == DeviceRole.MASTER and config:
        ...
        for i in range(config.model_parallel_size):
            if i != local_rank:
                ...
                if other_device and other_device.enabled:
                    # Since we can't easily get remote GPU info without API servers on workers,
                    # we'll show the device as part of the cluster but note it's a worker
                    device_info = {
                        "device_id": other_device.device_id,
                        "mlx_default_device": "Device(gpu, 0)",  # Assume GPU for Apple Silicon
                        "test_computation_time": None,  # Can't test remotely without API
                        "test_size": "N/A (worker node)",
                        "status": "Worker - GPU assumed active",
                        "rank": i,
                        "role": other_device.role.value,
                        "hostname": other_device.hostname,
                        "note": "Worker node - performance test not available via gRPC"
                    }
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    try:
        ...
        # Notify workers to participate using gRPC
        if communicator and hasattr(communicator, 'broadcast'):
            msg = {
                'type': 'chat',
            ...
            # Broadcast inference request to all workers
            communicator.broadcast(msg, root=local_rank)
            logger.info(f"Notified workers to participate in inference")
