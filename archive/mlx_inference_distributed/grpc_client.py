...
class DistributedInferenceClient:
    ...
    def connect_device(self, device_id: str, hostname: str, port: int) -> DeviceConnection:
        ...
        # Create channel with optimized options for MLX M4 cluster
        options = [
            ('grpc.max_send_message_length', 2048 * 1024 * 1024),  # 2GB for large tensors
            ('grpc.max_receive_message_length', 2048 * 1024 * 1024),  # 2GB for large tensors
            ('grpc.keepalive_time_ms', 20000),  # 20s keepalive
            ...
            ('grpc.dns_resolver', 'native'),  # Use native DNS
        ]
