#!/usr/bin/env python3
"""
Simple MLX Server for mini2.local - standalone version
"""
...
# Create FastAPI app
app = FastAPI(title="Mini2 MLX Server")
...
async def startup_event():
    ...
    logger.info(f"   Device: {'mini2.local' if 'mini2' in hostname else hostname}")
...
async def chat_completions(request: ChatRequest):
    ...
    try:
        ...
        if not MLX_AVAILABLE or model is None:
            # Fallback response
            response = f"Hello! I'm the mini2.local server. I received your message: '{request.messages[-1].content}'. MLX is {'not available' if not MLX_AVAILABLE else 'available but no model loaded'}."
        else:
            # Use MLX generation
            if request.stream:
...
async def stream_response(prompt: str, request: ChatRequest):
    ...
    try:
        ...
        if MLX_AVAILABLE and model is not None:
            ...
            # Fallback streaming
            message = "Hello from mini2.local! "
            for char in message:
...
if __name__ == "__main__":
    ...
    # Use port 8101 for mini2
    port = 8101
    
    logger.info(f"Starting Mini2 MLX Server on port {port}")
    if MLX_AVAILABLE:
        logger.info(f"MLX Device: {mx.default_device()}")
    else:
