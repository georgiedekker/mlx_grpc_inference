...
async def list_pipelines(x_api_key: str = Header(None)):
    """List all pipelines."""
    # API key validation
    if not x_api_key or x_api_key != "mlx-unified-key":
        raise HTTPException(status_code=401, detail="Invalid API key")
...
async def get_pipeline(pipeline_id: str, x_api_key: str = Header(None)):
...
async def create_from_template(
    ...
    return await create_pipeline(request, x_api_key)
