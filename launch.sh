...
EOF
    ...
    # Ensure /tmp/mlx_test exists on both machines
    mkdir -p /tmp/mlx_test
    ssh 192.168.5.2 "mkdir -p /tmp/mlx_test"
    
    # Copy server to both machines in same location
    echo -e "${YELLOW}Syncing server.py to /tmp/mlx_test on both machines...${NC}"
    cp server.py /tmp/mlx_test/
    scp -q server.py 192.168.5.2:/tmp/mlx_test/ || {
        echo -e "${RED}✗ Failed to copy server.py to mini2${NC}"
        exit 1
    }
    echo -e "${GREEN}✓ Server synced to both machines at /tmp/mlx_test${NC}"
    ...
    # Since mlx.launch uses system Python but we need uv environment,
    # we'll use mpirun with uv run instead
    # This ensures both nodes use their uv environments
    
    # Create a launcher script for each node
    cat > /tmp/mlx_test/run_server.sh << 'EOF'
#!/bin/bash
cd ~/Movies/mlx_grpc_inference
source .venv/bin/activate
python /tmp/mlx_test/server.py
EOF
    chmod +x /tmp/mlx_test/run_server.sh
    scp -q /tmp/mlx_test/run_server.sh 192.168.5.2:/tmp/mlx_test/
    
    # Use mpirun with explicit host specification
    nohup mpirun \
        -n 1 -host localhost bash /tmp/mlx_test/run_server.sh : \
        -n 1 -host 192.168.5.2 bash /tmp/mlx_test/run_server.sh \
